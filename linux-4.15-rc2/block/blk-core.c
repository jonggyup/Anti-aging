...
/* static is removed from the original version
 * by Jonggyu
 */
...
//  printk("Breakpoint #1");
  if (IS_ERR(req)) {
//    printk("Breakpoint Error #1");
    blk_queue_exit(q);
//    printk("Breakpoint Error #2");
    __wbt_done(q->rq_wb, wb_acct);
//    printk("Breakpoint Error #3");
    if (PTR_ERR(req) == -ENOMEM)
      bio->bi_status = BLK_STS_RESOURCE;
    else
      bio->bi_status = BLK_STS_IOERR;
    bio_endio(bio);
//    printk("Breakpoint Error #4");
    goto out_unlock;
  }
  ...
  req->frag_list = prev_rq;
  prev_rq = req;

  //  printk("Breakpoint #3");
  if (bio && bio->fragmented == 100 && bio->frag_list != NULL)
  {
    printk("In if condition statement filesystem name");
    printk("Breakpoint #4");
    bio = bio->frag_list;
    printk("Breakpoint #7");
    goto new;
  ...
  if (plug) {
    ...
    spin_lock_irq(q->queue_lock);
//    printk("Breakpoint #11");
    add_acct_request(q, req, where);
//    printk("Breakpoint #12");
    __blk_run_queue(q);
out_unlock:
    spin_unlock_irq(q->queue_lock);
  }
...
/* #1 in block layer
 * Accounting I/Os and call generic_make_request for making a request_queue
 * Commented by Jonggyu
 */
